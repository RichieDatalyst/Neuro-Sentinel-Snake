...
if page == "Overview":
    ...
    | Classifier Comparison | RF / SVM / GBM + GridSearchCV | Model selection, feature importance |
    ...
    with col2:
        if gl is not None:
            st.metric("Total Steps", f"{len(gl):,}")
        elif ep is not None:
            st.metric("Total Steps", f"{int(ep['total_steps'].sum()):,}")
        else:
    ...
    if os.path.exists(C.IMITATION_MODEL_PATH):
        try:
            ...
            _ytr_enc = encode_actions(_ytr)
            _yte_enc = encode_actions(_yte)
            ...
            c2.metric("Test accuracy",  f"{_te_acc:.1%}", help="On unseen mazes")
            c3.metric("Covariate shift", f"{(_tr_acc - _te_acc):.1%}",
                      delta=f"-{(_tr_acc - _te_acc):.1%}", delta_color="inverse",
                      help="How much accuracy drops on new mazes")
 
            st.markdown(
                f"The imitation MLP achieves **{_tr_acc:.1%}** on mazes it trained on "
                f"but only **{_te_acc:.1%}** on unseen mazes. "
                f"This **{(_tr_acc - _te_acc):.1%} covariate shift** shows that a model "
                f"memorising expert demonstrations fails when the environment changes, "
                f"even when the original expert (A*) would still succeed."
            )
        except Exception as _e:
